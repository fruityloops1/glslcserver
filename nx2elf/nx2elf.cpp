...
#undef ALLOC_SHDR_IF
    ...
    elf_size += (shstrtab.size + 0xFFF) & ~0xFFF;
    for (auto& seg : header.segments) {
      elf_size += (seg.mem_size + 0xFFF) & ~0xFFF;
    }
    ...
    size_t data_offset_cur = (shstrtab.offset + shstrtab.size + 0xFFF) & ~0xFFF;
    for (size_t i = 0; i < num_phdrs; i++) {
      ...
      if (i < kNumSegment) {
        ...
        memcpy(&elf[0] + phdr->p_offset, &image[seg.mem_offset],
               phdr->p_filesz);
        size_t left = (phdr->p_filesz + 0xFFF) & ~0xFFF - phdr->p_filesz;
        if (left)
          memset(&elf[0] + phdr->p_offset + phdr->p_filesz, 0, left);
        ...
        data_offset_cur += (phdr->p_filesz + 0xFFF) & ~0xFFF;
      } else if (i == kData + 1) {
